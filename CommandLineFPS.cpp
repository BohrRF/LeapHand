...
float fWallHeight = 0.6;          // Height of Wall
...
int main()
{
    ...
    auto keyTimeL1 = chrono::system_clock::now();
    auto keyTimeL2 = chrono::system_clock::now();
    auto keyTimeR1 = chrono::system_clock::now();
    auto keyTimeR2 = chrono::system_clock::now();
    bool ifKeepL = false;
	bool ifKeepR = false;
	...
	while (1)
	{
		...
		if ((GetAsyncKeyState(VK_LEFT) & 0x8000))
        {
            if(!(ifKeepR && ifKeepL))
            {
                if(!ifKeepL)
                {
                    keyTimeL1 = chrono::system_clock::now();
                    ifKeepL = true;
                }
                keyTimeL2 = chrono::system_clock::now();
                chrono::duration<float> ttemp = keyTimeL2 - keyTimeL1;
                fPlayerA -= (fSpeed * 0.5f) * fElapsedTime * (ttemp.count() + 0.3);
            }
            else
                keyTimeL1 = chrono::system_clock::now();
        }
		...
		if ((GetAsyncKeyState(VK_RIGHT) & 0x8000))
        {
            if(!(ifKeepR && ifKeepL))
            {
                if(!ifKeepR)
                {
                    keyTimeR1 = chrono::system_clock::now();
                    ifKeepR = true;
                }
                keyTimeR2 = chrono::system_clock::now();
                chrono::duration<float> ttemp = keyTimeR2 - keyTimeR1;
                fPlayerA += (fSpeed * 0.5f) * fElapsedTime * (ttemp.count() + 0.3);
            }
            else
                keyTimeR1 = chrono::system_clock::now();
		...
		for (int x = 0; x < nScreenWidth; x++)
		{
			...
			float fVFOV = fFOV * ((float)nScreenHeight / nScreenWidth);
			int nCeiling = 0.5 * nScreenHeight * (1 - 0.5 * fWallHeight / (fDistanceToWall * tan(0.5 * fVFOV)));
