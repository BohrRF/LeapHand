...
void control::hover(const int64_t& curTimeStamp)
{
    pauseTimeStamp = curTimeStamp;
    midi.stopMusic();
    playState = 3;
...
void control::calibrate(const int64_t& TimeStamp, const double& hand_amp, const double& hand_accel, const Fourier& fft)
{
    curAmp = hand_amp;
    curAccel = hand_accel;
    switch (cnt)
    ...
    case(1):
    case(2):
        cali_ary.push_back(hand_amp);
        cout << "Amp:" << hand_amp << endl;
        break;
    case(3):
        dif = ((TimeStamp - lastBeatTimeStamp) < (nextBeatTimeStamp - TimeStamp)) ? (TimeStamp - lastBeatTimeStamp) : (TimeStamp - nextBeatTimeStamp);
        dif /= 1e6;
        cali_ary.push_back(dif);
        cout << "dif:" << dif << endl;
        break;
    case(4):
    case(5):
    case(6):
        cali_ary.push_back(hand_accel);
        cout << "accel:" << hand_accel << endl;
        break;
    default:
        cnt = 0;
        cali_ary.clear();
...
const unsigned& control::calibrate_next()
{
    unsigned temp = (cali_ary.size() > 5 ? 5 : cali_ary.size());
    switch (cnt)
    {
    case(1):
        HAND_MIN = *std::min_element(cali_ary.end() - temp, cali_ary.end());
        break;
    case(2):
        HAND_MAX = *std::max_element(cali_ary.end() - temp, cali_ary.end());
        break;
    case(3):
        timeOffset = - 1e3 * std::accumulate(cali_ary.end() - temp, cali_ary.end(), 0.0) / 5;
        break;
    case(4):
        ACCEL_STD = std::accumulate(cali_ary.end() - temp, cali_ary.end(), 0.0) / 5;
        break;
    case(5):
        ACCEL_MIN = std::accumulate(cali_ary.end() - temp, cali_ary.end(), 0.0) / 5;
        break;
    case(6):
        ACCEL_MAX = std::accumulate(cali_ary.end() - temp, cali_ary.end(), 0.0) / 5;
        break;
    default:
        cnt = 0;
        cali_ary.clear();
...
void control::onBeat(const int64_t& TimeStamp, const double& hand_amp, const double& hand_accel ,const Fourier& fft)
{
    ...
    {
        resume(TimeStamp);
        bpmList.last->bpm = handBpm / gearFactor;
    }
    ...
    if (curAmp < HAND_MIN)
        curVelocityFactor = 0;
    else if(curAmp > HAND_MAX)
        curVelocityFactor = 2;
    else
        curVelocityFactor = 2 * (hand_amp - HAND_MIN) / (HAND_MAX - HAND_MIN);

    curAccel = hand_accel;
    if (curAccel < ACCEL_MIN)
        curSpanFactor = 0.5;
    else if (curAccel < ACCEL_STD)
        curSpanFactor = 0.5 * (curAccel - ACCEL_MIN) / (ACCEL_STD - ACCEL_MIN) + 0.5;
    else if(curAccel < ACCEL_MAX)
        curSpanFactor = (curAccel - ACCEL_STD) / (ACCEL_MAX - ACCEL_STD) + 1;
    else
        curSpanFactor = 2;
//system("cls");
...
//        printf("err:%+4.0fms /%4.0fms    ", dif * 1000, 60 * 1000 * gearFactor / handBpm);
        ...
        if(!speedFix)
        {
            if (abs(dif) <= 0.5 * 60 * gearFactor / handBpm)
            {
    ...
    */
    ...
    if(TimeStamp >= nextBeatTimeStamp)
    {
        ...
        if(node_ptr == beat_ptr->tickSet.begin())
            curBeatPos = beat_ptr - music.beatSet.begin() + 1;
        //cout << fft.calHightRatio(curAmp) << endl;
        if (fft.calHightRatio(curAmp) > 1.5 && playState == 2 && !isCalib)
        {
            hover(TimeStamp);
        }
    }
...
//            }
    ...
    if (playState == 2)
    {
        onPlayList.remove_if([&](const onPlayNote& val)
            {
                if (val.endTimeStamp <= TimeStamp)
                {
                    midi.noteOff(val.onNote.number, val.onNote.channel);
                    return true;
                }
                return false;
            });
